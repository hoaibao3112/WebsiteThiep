...
import { Check, Clipboard, Loader2, MessageCircle, Plus, Rocket, Search, Send, Trash2, Upload, Users } from "lucide-react";
...
export default function GuestsPage() {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    <div className="mb-6 flex flex-wrap items-start justify-between gap-4"><div><Link href="/dashboard/cards" className="text-sm text-stone-500 hover:text-stone-900">← Danh sách thiệp</Link><h1 className="mt-2 text-2xl font-semibold text-stone-950">Khách mời cá nhân</h1><p className="mt-1 text-sm text-stone-500">Tạo link riêng, chuẩn bị lời mời Zalo và theo dõi RSVP.</p></div><div className="flex gap-2"><Link href={`/dashboard/cards/${cardId}/share`} className="inline-flex min-h-10 items-center gap-2 rounded-lg bg-gradient-to-r from-amber-500 to-rose-500 px-3 text-sm font-semibold text-white shadow-sm shadow-amber-500/20 hover:shadow-md"><Rocket className="size-4"/>Gửi Hàng Loạt</Link><button onClick={() => setShowImport(true)} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-stone-200 bg-white px-3 text-sm font-medium"><Upload className="size-4"/>Nhập nhanh</button><button onClick={() => setShowAdd(true)} className="inline-flex min-h-10 items-center gap-2 rounded-lg bg-[#0068ff] px-3 text-sm font-semibold text-white"><Plus className="size-4"/>Thêm khách</button></div></div>
